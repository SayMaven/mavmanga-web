...
interface AuthorFilterProps {
  selectedAuthors: string[]; // KEMBALI JADI ARRAY
...
export default function AuthorFilter({ selectedAuthors, onChange }: AuthorFilterProps) {
  ...
  // Array untuk menyimpan detail author yang terpilih (Nama & ID)
  const [selectedDetails, setSelectedDetails] = useState<any[]>([]);

  const dropdownRef = useRef<HTMLDivElement>(null);
  const debounceRef = useRef<NodeJS.Timeout | null>(null);

  // 1. Fetch Detail saat Mount (Jika ada ID di URL tapi belum ada namanya)
  useEffect(() => {
    const fetchInitialDetails = async () => {
      // Cek jika ada ID terpilih tapi detailnya belum dimuat
  ...
  // 4. TOGGLE LOGIC (REVISI: MULTI SELECT)
  const toggleAuthor = (authorObj: any) => {
    ...
    if (isAlreadySelected) {
        ...
        // Cek dulu biar gak duplikat di tampilan selectedDetails
      ...
      {isOpen && (
        <div className="absolute z-50 top-full left-0 right-0 mt-1 bg-[#232529] border border-[#3b3e44] rounded shadow-2xl overflow-hidden flex flex-col max-h-96">
          ...
          <div className="overflow-y-auto flex-1 p-1 min-h-[100px]">
            ...
            {/* BAGIAN SELECTED (Muncul paling atas) */}
            {selectedDetails.length > 0 && (
                <div className="mb-2 border-b border-white/5 pb-2">
                    ...
                    {selectedDetails.map(author => (
                        <div 
                          key={author.id}
                          onClick={() => toggleAuthor(author)} // Klik untuk Hapus
            ...
            {isLoading ? (
                ...
                searchResults
                  .filter(res => !selectedAuthors.includes(res.id)) // Jangan tampilkan yg sudah dipilih
                  .map((author) => (
                    <div 
                      key={author.id}
                      onClick={() => toggleAuthor(author)} // Klik untuk Tambah
