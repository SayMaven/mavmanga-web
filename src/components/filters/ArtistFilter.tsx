...
export default function ArtistFilter({ selectedArtists, onChange }: ArtistFilterProps) {
  ...
  // 4. Toggle Logic
  const toggleArtist = (artistObj: any) => {
    const id = artistObj.id;
    const isAlreadySelected = selectedArtists.includes(id);

    if (isAlreadySelected) {
        onChange(selectedArtists.filter(a => a !== id));
        setSelectedDetails(prev => prev.filter(p => p.id !== id));
    } else {
        onChange([...selectedArtists, id]);
        if (!selectedDetails.find(p => p.id === id)) {
            setSelectedDetails(prev => [...prev, artistObj]);
        }
    }
  };

  const getLabel = () => {
    if (selectedArtists.length === 0) return 'Any';
    if (selectedArtists.length === 1 && selectedDetails.length > 0) return selectedDetails[0].attributes.name;
    return `${selectedArtists.length} Selected`;
  };

  return (
    <div className="relative" ref={dropdownRef}>
      <label className="block text-[10px] font-bold text-gray-400 uppercase mb-1.5">Artists</label>
        ...
        <div className="absolute z-50 top-full left-0 right-0 mt-1 bg-[#232529] border border-[#3b3e44] rounded shadow-2xl overflow-hidden flex flex-col max-h-96">
          ...
          <div className="overflow-y-auto flex-1 p-1 min-h-[100px]">
            ...
            {isLoading ? (
                ...
                searchResults
                  .filter(res => !selectedArtists.includes(res.id))
                  .map((artist) => (
                    <div 
                      key={artist.id}
                      onClick={() => toggleArtist(artist)}
                ...
                searchTerm && <div className="p-4 text-center text-xs text-gray-500">No artist found</div>
            )}
          </div>
        </div>
      )}
